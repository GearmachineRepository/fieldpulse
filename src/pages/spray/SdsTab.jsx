...
export default function SdsTab({ chemicals }) {
    ...
    return (
      <div>
        ...
        <div style={{ display: 'flex', gap: 10, marginBottom: 12 }}>
          {sel.labelUrl ? <a href={sel.labelUrl} target="_blank" rel="noopener noreferrer" style={{ ...btnStyle(C.accent, '#fff', { flex: 1, fontSize: 14, textDecoration: 'none', display: 'block', textAlign: 'center' }) }}>📋 View Label</a> : null}
          {sel.sdsUrl ? <a href={sel.sdsUrl} target="_blank" rel="noopener noreferrer" style={{ ...btnStyle(C.blue, '#fff', { flex: 1, fontSize: 14, textDecoration: 'none', display: 'block', textAlign: 'center' }) }}>☣️ View SDS</a> : null}
        </div>
      </div>
    )
  }
  ...
  return (
    <div>
      <input value={q} onChange={e => setQ(e.target.value)} placeholder="Search by name, type, or active ingredient" style={inputStyle({ marginBottom: 14 })} />
      {filtered.map(c => {
