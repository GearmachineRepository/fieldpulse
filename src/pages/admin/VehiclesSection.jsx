...
export default function VehiclesSection({ crews, onRefresh, showToast }) {
  ...
  const openForm = (v) => {
    if (v) {
      setEditItem(v)
      setVName(v.name)
      setVCrew(v.crew_name || '')
      setVPlate(v.license_plate || '')
      setVVin(v.vin || '')
      setVMakeModel(v.make_model || '')
      setVYear(v.year ? String(v.year) : '')
      setVTruckNum(v.truck_number || '')
    } else {
      setEditItem(null)
      setVName(''); setVCrew(''); setVPlate('')
      setVVin(''); setVMakeModel(''); setVYear(''); setVTruckNum('')
    }
    setShowForm(true)
  }
  ...
  const save = async () => {
    ...
    try {
      const data = {
        ...
        licensePlate: vPlate,
        vin: vVin,
        makeModel: vMakeModel,
        year: vYear,
        truckNumber: vTruckNum,
      }
    ...
    <div>
      ...
      {vehicles.map(v => (
        <div
          ...
          onClick={() => openForm(v)}
          onKeyDown={e => e.key === 'Enter' && openForm(v)}
            ...
            <div>
              ...
              {v.license_plate && (
                <div style={{ fontSize: 12, color: C.textLight }}>
                  Plate: {v.license_plate}{v.vin ? ` · VIN: ${v.vin}` : ''}
                </div>
              )}
            </div>
      ...
      {showForm && (
        <FormModal
          title={editItem ? 'Edit Vehicle' : 'New Vehicle'}
          onSave={save}
          onCancel={() => setShowForm(false)}
          onDelete={editItem ? handleFormDelete : undefined}
          saving={saving}
        >
          ...
          <div style={{ fontSize: 14, fontWeight: 800, marginTop: 8, marginBottom: 12, color: C.textMed }}>
            Vehicle Details
          </div>
