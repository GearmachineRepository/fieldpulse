...
import { getLocalDate, getLocalDow } from '@/lib/dateUtils.js'
        ...
        if (crew?.id) {
          try {
            ...
            const todayRoutes = routes.filter(
              (r) => r.dayOfWeek === getLocalDow() && r.stopCount > 0,
            )
            const stops = []
            for (const route of todayRoutes.slice(0, 3)) {
              try {
                ...
                console.error('Failed to load route day:', e.message)
              }
            }
            setTodayStops(stops)
          } catch (e) {
          ...
          try {
            ...
            console.error('Failed to load spray logs:', e.message)
          }
        }
      } catch (err) {
        console.error('FieldHome load error:', err)
      } finally {
        setLoading(false)
      }
    }
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, marginBottom: 24 }}>
          ...
          <button
            ...
            style={{
              ...
              cursor: 'pointer',
              border: 'none',
              background: todayRoster ? T.green : T.blue,
              fontFamily: T.font,
              display: 'flex',
              alignItems: 'center',
              gap: 10,
            ...
            todayStops.map((stop, i) => (
              <button
                ...
                style={{
                  ...
                  width: '100%',
                  borderRadius: 3,
                  border: `1.5px solid ${stop.isDone ? T.accentBorder : T.border}`,
                  marginBottom: 8,
                  alignItems: 'center',
                  cursor: 'pointer',
                  fontFamily: T.font,
              ...
              >
                  ...
                  {stop.isDone ? (
                    ...
                    <div
                      style={{
                        ...
                        height: 22,
                        borderRadius: 3,
                        background: stop.routeColor || T.accent,
                        display: 'flex',
                        alignItems: 'center',
                        justifyContent: 'center',
                        fontSize: 11,
                ...
                >
                  <Clock size={12} /> {stop.account.estimatedMinutes || stop.estimatedMinutes || 30}
                  m
                </div>
              </button>
            ))
              ...
              >
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  <div style={{ fontSize: 12, color: T.textLight }}>
                    {log.products?.length || 0} product
                    {(log.products?.length || 0) !== 1 ? 's' : ''} · {log.date}
                  </div>
                </div>
              </div>
            ))}
