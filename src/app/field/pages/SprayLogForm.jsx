...
export default function SprayLogForm({ onClose, onSubmitted }) {
  ...
  useEffect(() => {
    ...
    if (navigator.geolocation) {
      ...
      setWeather(getSimulatedWeather())
    } // eslint-disable-line react-hooks/set-state-in-effect -- Fallback when no geolocation
  }, [])
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (photos.length > 0 && result?.id) {
        try {
          await uploadPhotos(result.id, photos)
        } catch (e) {
      ...
      <div style={{ padding: '0 20px 100px', flex: 1, overflowY: 'auto' }}>
        ...
        {step === 1 && (
          <StepProducts
            ...
            totalMixVol={totalMixVol}
            setTotalMixVol={setTotalMixVol}
            mixUnit={mixUnit}
            setMixUnit={setMixUnit}
            targetPest={targetPest}
            setTargetPest={setTargetPest}
          />
        )}
        {step === 2 && (
          <StepConditions
  ...
  return (
    <div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, marginBottom: 16 }}>
        ...
        <ReadOnly
          label="Location"
          value={location ? 'GPS captured' : 'No GPS'}
          color={location ? T.accent : T.textLight}
        />
      </div>
...
function StepProducts({
  ...
  const addProduct = (chem) => {
    ...
    setProducts([
      ...products,
      { id: chem.id, name: chem.name, epa: chem.epa || 'N/A', amount: '', unit: 'oz' },
    ])
  ...
  return (
    <div>
      ...
      <button
        onClick={() => setShowChemSearch(true)}
        style={{
          width: '100%',
          padding: '12px',
          borderRadius: 3,
        ...
        >
          ...
          >
            ...
            >
              ...
              <input
                value={chemSearchQ}
                onChange={(e) => setChemSearchQ(e.target.value)}
                autoFocus
        ...
        <div>
          ...
          <div style={{ display: 'flex', gap: 6 }}>
            <input
              type="number"
              value={totalMixVol}
              onChange={(e) => setTotalMixVol(e.target.value)}
              placeholder="Amount"
              style={{ ...inp, flex: 1, marginBottom: 0 }}
            />
            <select
              value={mixUnit}
              onChange={(e) => setMixUnit(e.target.value)}
      ...
      {weather && (
        <div
          style={{
            ...
            background: T.card,
            borderRadius: 3,
            border: `1.5px solid ${windHigh ? T.red : T.border}`,
            marginBottom: 16,
          }}
        >
          {windHigh && (
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
            ...
            <WeatherField
              icon={WindIcon}
              label="Wind"
              value={`${weather.windSpeed || '--'} mph ${weather.windDir || ''}`}
              warn={windHigh}
            />
