...
import { getLocalDow, getDateForDow } from '@/lib/dateUtils.js'
  ...
  const loadDayStops = useCallback(async () => {
    ...
    for (const route of dayRoutes) {
      try {
        ...
        console.error('Failed to load route stops:', e.message)
      }
    }
    setDayStops(stops)
  ...
  const handleComplete = async (stop) => {
    if (stop.completion) {
      try {
        ...
        console.error('Failed to undo completion:', e.message)
      }
      return
    }
    try {
      ...
      console.error('Failed to complete stop:', e.message)
    }
  }

  const daysWithRoutes = new Set(
    allRoutes.filter((r) => r.dayOfWeek !== null && r.stopCount > 0).map((r) => r.dayOfWeek),
  )
          ...
          >
            ...
            <span
              style={{
                fontSize: 12,
                fontWeight: 600,
                color: completedCount === dayStops.length ? T.accent : T.textLight,
              }}
            >
          ...
          >
            ...
            <div style={{ fontSize: 15, fontWeight: 600, color: T.textMed }}>
              {isToday
                ...
                : `Nothing scheduled for ${DAY_LABELS[selectedDow]}`}
            </div>
          </div>
        ) : (
                  ...
                  >
                    ...
                    <span
                      style={{
                        ...
                        padding: '3px 8px',
                        borderRadius: 3,
                        background: `${ev.color || T.blue}15`,
                        color: ev.color || T.blue,
                      }}
                    >
                      {ev.type || 'Event'}
                    </span>
          ...
          onComplete={() => {
            ...
            setDetailStop((prev) =>
              prev
                ? {
                    ...prev,
                    completion: prev.completion ? null : { completedByName: employeeName },
                  }
                : null,
            )
          }}
...
function StopCard({ stop, index, onTap, onQuickComplete }) {
  ...
  return (
    <button
      ...
      onMouseEnter={(e) => {
        e.currentTarget.style.boxShadow = `0 0 0 2px ${T.accentBorder}`
      }}
      onMouseLeave={(e) => {
    ...
    >
      ...
      >
        ...
        {stop.account.contactPhone && (
          <div
            style={{ display: 'flex', alignItems: 'center', gap: 4, fontSize: 12, color: T.blue }}
          >
            <Phone size={12} /> Call
          </div>
        )}
        {stop.account.latitude && stop.account.longitude && (
          <div
            style={{ display: 'flex', alignItems: 'center', gap: 4, fontSize: 12, color: T.accent }}
          >
      ...
      {hasContact && (
        <div style={{ marginBottom: 14 }}>
          ...
          >
            ...
            {account.contactPhone && (
              <div style={{ fontSize: 13, color: T.textMed }}>{account.contactPhone}</div>
            )}
            {account.contactEmail && (
              <div style={{ fontSize: 13, color: T.textMed }}>{account.contactEmail}</div>
            )}
          </div>
        </div>
      )}
      ...
      <div style={{ marginBottom: 14 }}>
        ...
        >
          ...
          >
            ...
            <div style={{ fontSize: 13, fontWeight: 600, color: T.text }}>
              {stop.account.estimatedMinutes || stop.estimatedMinutes || 30}m
            </div>
          </div>
        </div>
      </div>
      ...
      <div>
        ...
        {loadingResources ? (
          ...
          >
            ...
            <div style={{ fontSize: 13, color: T.textLight }}>
              No resources attached to this site
            </div>
          </div>
        ) : (
  ...
  return (
    ...
    >
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        ...
        <div style={{ fontSize: 12, color: T.textLight }}>{r.categoryName || 'Resource'}</div>
      </div>
      {isLink ? (
