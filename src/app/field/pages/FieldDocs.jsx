    ...
    Promise.all([
      ...
      .then(([sprayLogs, fieldDocs]) => {
        ...
        const normalizedField = fieldDocs.map(fd => ({
          ...
          docType: fd.type,
          title: fd.title,
          subtitle: fd.type === "inspection"
            ? `${fd.metadata?.passCount || 0} pass · ${fd.metadata?.failCount || 0} fail`
            : (fd.body ? fd.body.slice(0, 60) + (fd.body.length > 60 ? "..." : "") : fd.employeeName || ""),
          date: fd.date,
          time: fd.time,
    ...
    <div>
      <div style={{ padding: "20px 20px 0" }}>
        ...
        <div style={{
          ...
          borderRadius: 3, padding: "12px 14px", border: `1px solid ${T.border}`, marginBottom: 12,
        }}>
        ...
        <div style={{ display: "flex", gap: 6, marginBottom: 16, overflowX: "auto" }}>
          {FILTER_PILLS.map(fp => (
            <button key={fp.key} onClick={() => setFilter(fp.key)} style={{
              padding: "6px 12px", borderRadius: 3, fontFamily: T.font,
        ...
        {loading ? (
          ...
          <div style={{
            ...
            <div style={{ fontSize: 13, color: T.textLight }}>
              {searchQ || filter !== "all" ? "Try a different search or filter." : "Tap the + button on home to create your first doc."}
            </div>
          </div>
        ) : (
          filtered.map(doc => {
            ...
            return (
              <button key={doc.id} onClick={() => setViewing(doc)} style={{
                ...
                <div style={{
                  width: 42, height: 42, borderRadius: 3, background: `${meta.color}10`,
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ display: "flex", justifyContent: "space-between", gap: 8 }}>
                    <div style={{ fontSize: 15, fontWeight: 600, color: T.text, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{doc.title}</div>
                    <div style={{ fontSize: 12, color: T.textLight, flexShrink: 0 }}>{doc.date}</div>
                  </div>
                  <div style={{ fontSize: 13, color: T.textMed, marginTop: 2, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                    {doc.subtitle}
                  </div>
                  <div style={{ display: "flex", alignItems: "center", gap: 8, marginTop: 4 }}>
                    ...
                    {doc.photoCount > 0 && (
                      <span style={{ fontSize: 12, color: T.textLight, display: "flex", alignItems: "center", gap: 3 }}>
                        <Camera size={11} /> {doc.photoCount}
                      </span>
                    )}
                    {doc.status === "flagged" && (
                      <span style={{ fontSize: 10, fontWeight: 600, color: T.red, display: "flex", alignItems: "center", gap: 3 }}>
...
function SprayLogDetail({ log, onClose }) {
  return (
    <div style={{ position: "fixed", inset: 0, background: T.bg, zIndex: 100, overflowY: "auto", fontFamily: T.font }}>
...
function NoteDetail({ doc, onClose }) {
  return (
    <div style={{ position: "fixed", inset: 0, background: T.bg, zIndex: 100, overflowY: "auto", fontFamily: T.font }}>
      ...
      <div style={{ padding: 20 }}>
        <div style={{ fontSize: 20, fontWeight: 600, marginBottom: 4, color: T.text }}>{doc.title}</div>
        <div style={{ fontSize: 13, color: T.textLight, marginBottom: 20 }}>{doc.date} at {doc.time}</div>
        ...
        {doc.body && (
          <div style={{
            ...
            border: `1px solid ${T.border}`, fontSize: 14, color: T.textMed, lineHeight: 1.6, marginBottom: 20,
          }}>{doc.body}</div>
        )}
  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: T.bg, zIndex: 100, overflowY: "auto", fontFamily: T.font }}>
      ...
      <div style={{ padding: 20 }}>
        ...
        <div style={{
          display: "flex", gap: 12, padding: "12px 16px", background: T.card,
          borderRadius: 3, border: `1px solid ${T.border}`, marginBottom: 16, fontSize: 13, fontWeight: 600,
        }}>
        ...
        <div style={{ marginBottom: 20 }}>
          {checklist.map((c, i) => (
            <div key={i} style={{
              ...
              padding: "10px 14px", background: T.card, borderRadius: 3,
              border: `1px solid ${c.status === "fail" ? T.red : T.border}`, marginBottom: 6,
            }}>
              <div style={{ flex: 1 }}>
              ...
              <div style={{
                fontSize: 11, fontWeight: 600, textTransform: "uppercase", flexShrink: 0, marginLeft: 8,
        ...
        {doc.body && (
          <div style={{
            ...
            <div style={{ fontSize: 12, fontWeight: 600, color: T.textLight, textTransform: "uppercase", marginBottom: 6 }}>Notes</div>
            {doc.body}
          </div>
        )}
...
function PhotoGrid({ photos }) {
  ...
  return (
    <div style={{ marginBottom: 20 }}>
      <div style={{ fontSize: 14, fontWeight: 600, marginBottom: 10, color: T.text }}>Photos ({photos.length})</div>
