      ...
      request(endpoint)
        .then((items) => {
          ...
          const activeItems = (items || []).filter(
            (i) => i.active !== false && i.status !== 'Retired',
          )
      ...
      <div style={{ display: 'flex', gap: 4, padding: '0 20px 16px' }}>
        {STEPS.map((s, i) => (
          ...
          >
            ...
            />
            <span
              style={{ fontSize: 10, fontWeight: 600, color: i <= step ? T.accent : T.textLight }}
            >
              {s}
            </span>
          </div>
      ...
      <div style={{ padding: '0 20px 100px', flex: 1, overflowY: 'auto' }}>
        {step === 0 && (
          <StepInfo
            ...
            assetId={assetId}
            assetName={assetName}
            assetManual={assetManual}
            assetList={assetList}
            loadingAssets={loadingAssets}
            setAssetId={setAssetId}
            setAssetName={setAssetName}
            setAssetManual={setAssetManual}
          />
        )}
        {step === 1 && (
          <StepChecklist
        ...
        {step < 3 ? (
          ...
          <button
            ...
            style={{
              ...
              border: 'none',
              cursor: 'pointer',
              background: failCount > 0 ? T.red : T.accent,
              color: T.card,
              fontSize: 15,
              fontWeight: 600,
              fontFamily: T.font,
          ...
          >
            {submitting ? (
              ...
              <>
                <Check size={18} /> Submit Inspection
              </>
            )}
          </button>
        )}
...
function StepInfo({
  ...
  const selectAsset = (item) => {
    ...
    if (!title || title === 'Vehicle Pre-Trip' || title === 'Equipment Check') {
      setTitle(
        `${INSPECTION_TEMPLATES.find((t) => t.key === template)?.label || 'Inspection'} — ${name}`,
      )
    }
  }
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, marginBottom: 20 }}>
        {INSPECTION_TEMPLATES.map((t) => (
          ...
          >
            <ClipboardList
              size={20}
              color={template === t.key ? T.accent : T.textLight}
      ...
      {needsAsset && (
        <div style={{ marginBottom: 16 }}>
          ...
          {!assetManual ? (
            <>
              {loadingAssets ? (
                <div style={{ fontSize: 13, color: T.textLight, padding: '10px 0' }}>
                  Loading {assetLabel.toLowerCase()}s...
                </div>
              ) : assetList.length > 0 ? (
                ...
                <div style={{ fontSize: 13, color: T.textLight, padding: '10px 0' }}>
                  No {assetLabel.toLowerCase()}s found in the system.
                </div>
              )}
              <button
                onClick={() => setAssetManual(true)}
                style={{
                  border: 'none',
                  background: 'none',
                  cursor: 'pointer',
              ...
              >
                {assetList.length > 0
                  ? `${assetLabel} not listed? Type it in manually`
                  : 'Enter manually'}
              </button>
            </>
          ) : (
        ...
        >
          ...
          <div style={{ display: 'flex', gap: 6 }}>
            {[
              ...
              <button
                key={opt.key}
                onClick={() => updateItem(i, 'status', opt.key)}
                style={{
                  flex: 1,
                  padding: '8px 0',
                  borderRadius: 3,
...
function StepPhotos({ photos, setPhotos, photoPreviews, setPhotoPreviews }) {
  ...
  const handlePhotos = (e) => {
    ...
    files.forEach((f) => {
      ...
      r.onload = (ev) =>
        setPhotoPreviews((prev) => [...prev, { name: f.name, url: ev.target.result }])
      r.readAsDataURL(f)
    })
  }
  ...
  return (
    <div>
      ...
      <div style={{ fontSize: 13, color: T.textLight, marginBottom: 20 }}>
        Document any issues or conditions (optional)
      </div>
  ...
  return (
    <div>
      ...
      {failCount > 0 && (
        ...
        >
          <XCircle size={16} /> {failCount} item{failCount !== 1 ? 's' : ''} failed — this
          inspection will be flagged
        </div>
      )}
      ...
      <ReviewSection title="Inspection Info">
        ...
        {assetName && (
          <ReviewRow label={assetType === 'vehicle' ? 'Vehicle' : 'Equipment'} value={assetName} />
        )}
        ...
        <ReviewSection title={`Photos (${photos.length})`}>
          <div style={{ display: 'flex', gap: 6 }}>
            {photos.map((_, i) => (
              ...
              >
                <Camera size={16} color={T.textLight} />
              </div>
            ))}
          </div>
        </ReviewSection>
