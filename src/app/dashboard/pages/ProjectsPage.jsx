    ...
    return (
      <div className={s.detailRead}>
        ...
        <div className={s.fieldGrid}>
          ...
          <ReadField label="Group" value={group ? group.name : 'None'} color={group?.color} />
        </div>
...
function ReadField({ label, value, mono, color }) {
  return (
    <div className={s.readFieldWrap}>
      ...
      <div className={`${s.readValue} ${mono ? s.mono : ''}`} style={color ? { color } : undefined}>
        {value || '\u2014'}
      </div>
    </div>
  )
...
function ResourcesTab({ projectId, toast }) {
  ...
  const load = async () => {
    ...
    let allRes = []
    try {
      allRes = await getResources()
    } catch (err) {
      console.error('Failed to load resources:', err)
    }
    try {
      linkedRes = await getAccountResources(projectId)
    } catch (err) {
      ...
      <div className={s.resourcesHeader}>
        <div>
          <div className={s.resourcesCount}>
            <span className={s.mono}>{linked.length}</span> resource{linked.length !== 1 ? 's' : ''}{' '}
            linked
          </div>
          <div className={s.resourcesSub}>Crews see these when visiting this jobsite</div>
        </div>
            ...
            <div key={r.id} className={s.linkedRow}>
              ...
              <div className={s.resourceInfo}>
                ...
                <div className={s.resourceMeta}>
                  <span className={s.resourceBadge}>
                    {r.resourceType === 'link' ? 'Link' : 'File'}
                  </span>
                  {r.fileSize && <span className={s.mono}>{formatFileSize(r.fileSize)}</span>}
                  {r.createdAt && (
                    <span className={s.mono}>{new Date(r.createdAt).toLocaleDateString()}</span>
                  )}
                </div>
              </div>
            ...
            available.map((r) => (
              <button key={r.id} onClick={() => onSelect(r.id)} className={s.pickerItem}>
                <div
                  className={s.resourceIcon}
                  style={{
                    background: r.categoryColor ? `${r.categoryColor}15` : 'var(--blu-dim)',
                  }}
                >
                  {r.resourceType === 'link' ? (
                    <ExternalLink size={16} color={r.categoryColor || 'var(--blu)'} />
          ...
          {items.map((item) => (
            <div key={item.id} className={s.manageRow}>
              ...
              <div className={s.manageInfo}>
                ...
                <div className={s.manageCount}>
                  {item.count} item{item.count !== 1 ? 's' : ''}
                </div>
              </div>
              <button
                onClick={() => setEditingItem(item)}
                className={s.manageActionBtn}
                title="Edit"
              >
