      ...
      {isEmpty ? (
        <div className={s.emptyState}>
          ...
          <div className={s.emptyDesc}>
            Add employee certifications to track expiration dates, receive renewal alerts, and
        ...
        <CertTable filtered={filtered} onEdit={setEditing} onDelete={setConfirmDelete} />
      ) : (
            ...
            >
              ...
              <td className={`${ts.td} ${ts.tdMono}`}>
                {cert.expiryStatus.days !== undefined ? (
                  <span
                    style={{
                      color: `var(--${cert.expiryStatus.variant === 'green' ? 'grn' : cert.expiryStatus.variant === 'amber' ? 'amb' : 'red'})`,
                    }}
                  >
                    {cert.expiryStatus.days < 0
                      ? `${Math.abs(cert.expiryStatus.days)}d ago`
                      : `${cert.expiryStatus.days}d`}
                  </span>
                ) : (
                  '\u2014'
                )}
              ...
              <td className={ts.td}>
                <StatusBadge variant={cert.expiryStatus.variant}>
                  {cert.expiryStatus.label}
                </StatusBadge>
              </td>
