...
export default function EquipmentPage() {
  ...
  const categoryNames =
    categories.data.length > 0 ? categories.data.map((c) => c.name) : FALLBACK_CATEGORIES
  ...
  const filtered = equipment.data.filter((e) => {
    if (searchQ) {
      ...
      if (!e.name.toLowerCase().includes(q) && !(e.serial_number || '').toLowerCase().includes(q))
        return false
    }
          ...
          {filtered.length === 0 ? (
            ...
            filtered.map((item) => (
              ...
              >
                ...
                <td className={`${ts.td} ${ts.tdMono}`}>{item.serial_number || '\u2014'}</td>
                <td className={ts.td}>
                  {item.crew_name || <span className={ts.tdMuted}>Unassigned</span>}
                </td>
                <td className={`${ts.td} ${ts.tdMono}`}>
                  {item.last_service
                    ? new Date(item.last_service).toLocaleDateString('en-US', {
                        month: 'short',
                        day: 'numeric',
                        year: 'numeric',
                      })
      ...
      {tab === 'info' && (
        <div className={s.fieldList}>
          ...
          <DetailField
            ...
            label="Last Service"
            value={
              item.last_service ? new Date(item.last_service).toLocaleDateString('en-US') : null
            }
            mono
          />
