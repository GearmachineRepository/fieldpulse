...
export default function DashboardHome() {
  ...
  // ── Save pins ──
  const savePins = useCallback(async (newPins) => {
    setPins(newPins)
    try {
          ...
          <div className={s.card}>
            ...
            {loadingAttendance || crews.loading ? (
              <div className={s.skeletonRows}>
                {Array.from({ length: 4 }).map((_, i) => <div key={i} className={s.skeletonRow} />)}
              </div>
      ...
      case "active_projects":
        return (
          <div className={s.card}>
            ...
            {accounts.loading ? (
              <div className={s.skeletonRows}>
                {Array.from({ length: 3 }).map((_, i) => <div key={i} className={s.skeletonRow} />)}
              </div>
              ...
              <div className={s.emptyState}>
                ...
                <button className={s.emptyCta} onClick={() => onNavigate?.("projects")}>Add Project</button>
              </div>
            ) : (
    ...
    <div>
      ...
      {topRow.length > 0 && (
        <div className={s.twoCol}>
          {topRow.map(key => <div key={key}>{renderCard(key)}</div>)}
        </div>
      )}
      ...
      {midRow.length > 0 && (
        <div className={s.threeCol}>
          {midRow.map(key => <div key={key}>{renderCard(key)}</div>)}
        </div>
      )}
      ...
      {restRows.length > 0 && (
        <div className={s.twoCol}>
          {restRows.map(key => <div key={key}>{renderCard(key)}</div>)}
        </div>
      )}
      ...
      {customizeOpen && (
        <CustomizeModal
          pins={pins}
          isEnabled={isEnabled}
          onSave={(newPins) => { savePins(newPins); setCustomizeOpen(false) }}
