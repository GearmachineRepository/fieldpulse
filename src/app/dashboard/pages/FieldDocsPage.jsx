    ...
    <div>
      ...
      <div className={s.filters}>
        ...
        <div className={s.selectWrap}>
          ...
          >
            {FILTER_TYPES.map(ft => <option key={ft.key} value={ft.key}>{ft.label}</option>)}
          </select>
        </div>
        {crewNames.length > 1 && (
          <div className={s.selectWrap}>
      ...
      {isLoading ? <LoadingSpinner /> :
       filtered.length === 0 ? (
        <EmptyMessage text={searchQ || filterCrew || filterType ? "No docs match your filters." : "No field docs yet. Crews submit them from the field app."} />
      ) : (
            ...
            return (
              <div key={doc.id}>
                ...
                >
                  ...
                  <div>
                    <span className={s.typeBadge} style={{ background: `${meta.color}15`, color: meta.color }}>
                  ...
                  <div className={s.cellMed}>
                    {doc.photoCount > 0 ? doc.photoCount : "—"}
                  </div>
                  <div>
                    <span className={`${s.statusBadge} ${doc.status === "flagged" ? s.statusFlagged : ""}`}>
                ...
                >
                  ...
                  <div className={s.mobileContent}>
                    ...
                    <div className={s.mobileMeta}>
                      ...
                      {doc.photoCount > 0 ? ` · ${doc.photoCount} photo${doc.photoCount !== 1 ? "s" : ""}` : ""}
                    </div>
                  </div>
    ...
    <Modal title="Spray Log Detail" onClose={onClose} size="lg">
      <div className={s.modalHeader}>
        <div>
          ...
          <div className={s.modalSubtitle}>{log.date} at {log.time}</div>
        </div>
      ...
      {log.weather && (log.weather.temp || log.weather.windSpeed) && (
        <div className={s.section}>
          ...
          <div className={s.weatherBar}>
            {log.weather.temp && <div className={s.weatherItem}><Thermometer size={14} color="var(--t3)" /><span className={s.weatherBold}>{log.weather.temp}°F</span></div>}
            {log.weather.humidity && <div className={s.weatherItem}><Droplets size={14} color="var(--t3)" /><span>{log.weather.humidity}%</span></div>}
            {log.weather.windSpeed && <div className={s.weatherItem}><Wind size={14} color="var(--t3)" /><span>{log.weather.windSpeed} mph {log.weather.windDir || ""}</span></div>}
            {log.weather.conditions && <div className={s.weatherItem}><Cloud size={14} color="var(--t3)" /><span>{log.weather.conditions}</span></div>}
          </div>
        </div>
      )}
        ...
        {log.products.length === 0 ? (
          ...
          <div className={s.productList}>
            {log.products.map((p, i) => (
              <div key={i} className={s.productRow} style={i < log.products.length - 1 ? { borderBottom: "1px solid var(--bd)" } : undefined}>
                <div>
      ...
      {log.members && log.members.length > 0 && (
        <div className={s.section}>
          ...
          <div className={s.memberList}>
            {log.members.map((m, i) => <div key={i} className={s.memberChip}>{m.employee_name || m.name}</div>)}
          </div>
        </div>
      )}
          ...
          <div className={s.photoGrid}>
            {log.photos.map((photo, i) => (
              <a key={i} href={`/uploads/${photo.filename}`} target="_blank" rel="noopener noreferrer" className={s.photoLink}>
                <img src={`/uploads/${photo.filename}`} alt={photo.original_name || `Photo ${i + 1}`} className={s.photoImg} />
              </a>
            ))}
          </div>
        </div>
    ...
    <Modal title="General Note" onClose={onClose} size="lg">
      <div className={s.modalHeader}>
        <div>
          ...
          <div className={s.modalSubtitle}>{doc.date} at {doc.time}</div>
        </div>
      ...
      <div className={s.infoGrid}>
        ...
        {doc.location && <InfoCell icon={MapPin} label="Location" value={<AddressLink location={doc.location} />} />}
      </div>
        ...
        <div className={s.section}>
          ...
          <div className={s.photoGrid}>
            {doc.photos.map((photo, i) => (
              <a key={i} href={`/uploads/${photo.filename}`} target="_blank" rel="noopener noreferrer" className={s.photoLink}>
                <img src={`/uploads/${photo.filename}`} alt={photo.originalName || `Photo ${i + 1}`} className={s.photoImg} />
              </a>
            ))}
          </div>
        </div>
  ...
  return (
    <Modal title="Inspection Report" onClose={onClose} size="lg">
      ...
      <div className={s.infoGrid}>
        ...
        <InfoCell icon={Users} label="Crew" value={doc.crewName || "—"} />
        {doc.location && <InfoCell icon={MapPin} label="Location" value={<AddressLink location={doc.location} />} />}
        <InfoCell icon={ClipboardList} label="Template" value={doc.metadata?.template || "Custom"} />
      </div>
          ...
          {checklist.map((c, i) => (
            ...
            >
              <div>
                ...
                {c.note && <div style={{ fontSize: "var(--text-xs)", color: "var(--red)", marginTop: 2 }}>{c.note}</div>}
              </div>
              <div style={{
                fontSize: "var(--text-xs)",
                fontWeight: 600,
                textTransform: "uppercase",
                color: c.status === "pass" ? "var(--amb)" : c.status === "fail" ? "var(--red)" : "var(--t3)",
              }}>{c.status}</div>
            </div>
          ))}
        </div>
