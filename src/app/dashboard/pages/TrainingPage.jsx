...
export default function TrainingPage() {
  ...
  const defaultType =
    tab === 'tailgate' ? 'Tailgate Meeting' : tab === 'video' ? 'Video Training' : ''
  ...
  return (
    <>
      ...
      >
        ...
        {tab === 'sessions' && (
          <SessionsTab data={sessions.data} onSelect={handleSelect} onEdit={setEditing} />
        )}
        {tab === 'tailgate' && (
          <TailgateTab
            data={tailgate.data}
        ...
        {tab === 'video' && <VideoTab data={video.data} onSelect={handleSelect} />}
      </PageShell>
...
function SessionsTab({ data, onSelect, onEdit: _onEdit }) {
  ...
  const filtered = data.filter((item) => {
    if (searchQ) {
      ...
      if (!item.title.toLowerCase().includes(q) && !(item.trainer || '').toLowerCase().includes(q))
        return false
    }
          ...
          filtered.map((item) => {
            ...
            return (
              <tbody key={item.id}>
                ...
                >
                  <td className={`${ts.td} ${ts.tdMono}`}>{formatDate(item.training_date)}</td>
                  <td className={ts.td}>
    ...
    <div className={s.videoGrid}>
      {data.map((item) => (
        <div
          ...
          tabIndex={0}
          onKeyDown={(e) =>
            (e.key === 'Enter' || e.key === ' ') && (e.preventDefault(), onSelect(item.id))
          }
        >
    ...
    <div className={s.detailContent}>
      ...
      <div className={s.fieldList}>
        ...
        <DetailField
          icon={Clock}
          label="Duration"
          value={session.duration_hours ? `${session.duration_hours} hours` : null}
        />
  ...
  return (
    <Modal title={isEdit ? 'Edit Training Session' : 'Log Training Session'} onClose={onClose}>
      <FormField
        ...
        onChange={setTitle}
        autoFocus
        placeholder="e.g. Heat Illness Prevention"
      />
            ...
            {filtered.length === 0 ? (
              ...
              filtered.map((e) => (
                <button
                  ...
                  disabled={saving}
                >
                  <span className={s.signoffPickName}>
