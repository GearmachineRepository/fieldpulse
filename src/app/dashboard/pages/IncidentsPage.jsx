...
export default function IncidentsPage() {
  ...
  const stats = useMemo(() => {
    ...
    return {
      ...
      openUnresolved: incidents.data.filter(
      ...
      >
        ...
        <div className={s.statsRow}>
          <StatCard
            ...
            value={stats.totalYTD}
            color="var(--amb)"
            icon={AlertTriangle}
          />
          <StatCard
            label="Open / Unresolved"
            value={stats.openUnresolved}
            color="var(--amb)"
            icon={Clock}
          />
          <div className={stats.withInjury > 0 ? s.statRedTint : undefined}>
            <StatCard
              label="Involving Injury"
              value={stats.withInjury}
              color="var(--red)"
              icon={Shield}
            />
          </div>
        ...
        >
          {filtered.length === 0 ? (
            ...
            filtered.map((item) => {
              const rowAccent = item.injury_occurred
                ? s.rowInjury
                ...
                >
                  ...
                  <td className={ts.td}>
                    <StatusBadge variant={getSeverityVariant(item.severity)}>
                      {(item.severity || 'low').charAt(0).toUpperCase() +
                        (item.severity || 'low').slice(1)}
                    </StatusBadge>
                  </td>
...
function IncidentWizard({ initialData, employees, onClose, onSave }) {
  ...
  const canAdvance = useMemo(() => {
    switch (step) {
      case 0:
        return !!(form.incidentType && form.title.trim() && form.incidentDate)
      case 1:
        return true // parties step always allows advance
      case 2:
  ...
  return (
    <div className={s.wizardStep}>
      ...
      <FormField
        ...
        onChange={(v) => updateForm({ incidentDate: v })}
        type="datetime-local"
      />
      ...
      <div className={s.checkRow}>
        <label className={s.checkLabel}>
          <input
            ...
            onChange={(e) => updateForm({ injuryOccurred: e.target.checked })}
      ...
      {form.injuryOccurred && (
        <TextareaField
          ...
          onChange={(v) => updateForm({ injuryDescription: v })}
    ...
    <div className={s.wizardStep}>
        ...
        <>
          ...
          <div className={s.formRow}>
            <FormField
              ...
              onChange={(v) => updateForm({ otherDriverName: v })}
              placeholder="Full name"
            />
            <FormField
              label="Phone"
              value={form.otherDriverPhone}
              onChange={(v) => updateForm({ otherDriverPhone: v })}
              placeholder="(555) 000-0000"
            />
          </div>
          <FormField
            label="Insurance Info"
            value={form.otherDriverInsurance}
            onChange={(v) => updateForm({ otherDriverInsurance: v })}
            placeholder="Insurance company and policy #"
          />
        </>
      )}
      ...
      {isEquipment && (
        <TextareaField
          ...
          onChange={(v) => updateForm({ equipmentDescription: v })}
          placeholder="Equipment make, model, ID number..."
          rows={2}
        />
      )}
      ...
      <TextareaField
        ...
        onChange={(v) => updateForm({ witnesses: v })}
        ...
        <div className={s.checkRow}>
          <label className={s.checkLabel}>
            <input
              ...
              onChange={(e) => updateForm({ medicalTreatment: e.target.checked })}
              className={s.checkbox}
            />
  ...
  return (
    <div className={s.wizardStep}>
      ...
      <div className={s.wizardStepDesc}>
        Photos will be uploaded after the report is created. Use the labeled slots below as a guide
        for what to capture.
      </div>
    ...
    <div className={s.wizardStep}>
      ...
      <div className={s.checkRow}>
        <label className={s.checkLabel}>
          <input
            ...
            onChange={(e) => updateForm({ supervisorNotified: e.target.checked })}
            className={s.checkbox}
          />
          Supervisor notified
        </label>
      ...
      {form.supervisorNotified && (
        <FormField
          ...
          onChange={(v) => updateForm({ supervisorName: v })}
      ...
      <div className={s.checkRow}>
        <label className={s.checkLabel}>
          <input
            type="checkbox"
            checked={form.lawEnforcement}
            onChange={(e) => updateForm({ lawEnforcement: e.target.checked })}
            className={s.checkbox}
          />
      ...
      {form.lawEnforcement && (
        <FormField
          ...
          value={form.lawEnforcementReport}
          onChange={(v) => updateForm({ lawEnforcementReport: v })}
          placeholder="Police report number and department"
        />
      )}
      ...
      <div className={s.checkRow}>
        <label className={s.checkLabel}>
          <input
            type="checkbox"
            checked={form.emergencyMedical}
            onChange={(e) => updateForm({ emergencyMedical: e.target.checked })}
            className={s.checkbox}
          />
      ...
      <TextareaField
        ...
        onChange={(v) => updateForm({ followUpActions: v })}
      ...
      <div className={s.confirmBox}>
        <label className={s.checkLabel}>
          ...
          />
          <span>
            I confirm all information in this report is accurate to the best of my knowledge.
          </span>
        </label>
      </div>
    </div>
      ...
      <div className={s.reviewCard}>
        ...
        <div className={s.reviewRow}>
          ...
          <span className={s.mono}>
            {form.incidentDate ? new Date(form.incidentDate).toLocaleString() : '\u2014'}
          </span>
        </div>
        <div className={s.reviewRow}>
      ...
      {tab === 'info' && (
        <div className={s.fieldList}>
          ...
          <DetailField
            icon={MapPin}
            label="Location"
            value={item.location ? <AddressLink location={item.location} /> : '\u2014'}
          />
          <DetailField
            icon={Shield}
            label="Severity"
            value={
              item.severity ? item.severity.charAt(0).toUpperCase() + item.severity.slice(1) : 'Low'
            }
          />
          ...
          <DetailField
            ...
            label="Status"
            value={
              item.status ? item.status.charAt(0).toUpperCase() + item.status.slice(1) : 'Open'
            }
          />
          {item.description && (
          ...
          {item.injury_occurred && (
            <DetailField
              icon={Shield}
              label="Injury Description"
              value={item.injury_description || 'Injury occurred (no details provided)'}
            />
          )}
        ...
        <div className={s.fieldList}>
          {item.investigation_notes ? (
            <DetailField
              ...
              value={item.investigation_notes}
            />
          ) : (
            <div className={s.emptyTab}>
              ...
              <div className={s.emptyTabDesc}>
                Edit the incident to add investigation notes and corrective actions.
              </div>
            </div>
          )}
    ...
    <div className={s.photosTab}>
      {locked && (
        <div className={s.lockedNotice}>
          <Lock size={14} /> Report is locked — photos cannot be added or removed.
        </div>
      )}
      {!locked && (
