...
import { Modal, ModalFooter, ConfirmModal, FormField } from '../components/PageUI.jsx'
      ...
      >
        <div className={s.grid}>
          {crews.data.map((crew) => {
            ...
            return (
              <div key={crew.id} className={s.card} onClick={() => setDetailCrew(crew)}>
                <div className={s.cardTop}>
                  ...
                  <div className={s.cardInfo}>
                    ...
                    <div className={s.cardMeta}>
                      <span className={s.mono}>{members.length}</span> member
                      {members.length !== 1 ? 's' : ''}
                      {lead && (
                        <>
                          {' · Lead: '}
                ...
                {members.length === 0 && <div className={s.noMembers}>No members assigned</div>}
              </div>
            )
          })}
        </div>
    ...
    <div className={s.detail}>
      ...
      <div className={s.detailHeader}>
        <div className={s.detailInfo}>
          <div className={s.detailMeta}>
            <span className={s.mono}>{members.length}</span> member{members.length !== 1 ? 's' : ''}
            {lead && (
              <>
                {' · Lead: '}
                <span className={s.leadName}>
          ...
          {members.map((emp) => (
            <div key={emp.id} className={s.memberRow}>
              ...
              <div className={s.memberRowInfo}>
                ...
                {emp.is_crew_lead && <StatusBadge variant="amber">Lead</StatusBadge>}
              </div>
      ...
      <div className={s.detailSection}>
        ...
        <div className={s.memberRows}>
          {unassigned.map((emp) => (
            <div key={emp.id} className={s.memberRow}>
              <div className={s.memberAvatarLg} style={{ background: 'var(--s3)' }}>
                {emp.first_name[0]}
                {emp.last_name[0]}
              </div>
              <div className={s.memberRowInfo}>
                ...
                {emp.crew_name && <span className={s.memberRowCrew}>{emp.crew_name}</span>}
              </div>
              <button
