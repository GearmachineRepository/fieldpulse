...
export function ModalFooter({
  ...
  return (
    <div className={s.modalFooter}>
      <div>
        {onDelete && (
          <button onClick={onDelete} className={s.btnDelete}>
            {deleteLabel}
          </button>
        )}
      </div>
    ...
    <div onClick={(e) => e.target === e.currentTarget && onCancel()} className={s.confirmOverlay}>
      <div className={s.confirmPanel} role="alertdialog" aria-label={title}>
        ...
        <div className={s.confirmActions}>
          <button onClick={onCancel} className={s.btnConfirmCancel}>
            Cancel
          </button>
          <button
            onClick={onConfirm}
            className={s.btnConfirmAction}
...
export function DropdownFilter({ label, options, value, onChange, onCreateNew }) {
  ...
  const activeLabel = value
    ? options.find((o) => (o.value ?? o.id) === value)?.label ||
      options.find((o) => (o.value ?? o.id) === value)?.name ||
      label
    : label
          ...
          <div className={s.dropdownOptions}>
            ...
            {filtered.map((o) => {
              ...
              return (
                ...
                >
                  {o.color && (
                    <div className={s.dropdownColorDot} style={{ background: o.color }} />
                  )}
                  {oLabel}
                </button>
              )
