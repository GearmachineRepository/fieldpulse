...
export default function DashboardSidebar({
  ...
  const section = useMemo(() => SECTIONS.find((sec) => sec.key === activeSection), [activeSection])
    ...
    <>
      ...
      <nav className={`${s.sidebar} ${open ? s.open : ''}`} aria-label="Main navigation">
        ...
        <div className={s.desktopNav}>
          {section && <div className={s.sectionHeader}>{section.label.toUpperCase()}</div>}
            ...
            {pages.map((page) => {
              ...
              return (
                ...
                >
                  {page.icon && <page.icon size={14} className={s.navIcon} />}
                  {page.label}
                  {page.badge && <span className={`${s.count} ${s.countWarn}`}>2</span>}
