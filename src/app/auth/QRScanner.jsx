...
export default function QRScanner({ open, onClose, onScan }) {
  ...
  useEffect(() => {
    ...
    async function startCamera() {
      ...
      if (!isSecureContext) {
        setError(
          "Camera requires a secure connection (HTTPS). You're currently on HTTP — ask your admin to enable HTTPS, or enter the code manually.",
        )
        return
      }
      ...
      if (!navigator.mediaDevices || !navigator.mediaDevices.getUserMedia) {
        ...
        if (!legacyGetUserMedia) {
          setError(
            'Camera is not supported on this browser. Please use Chrome, Safari, or Firefox — or enter the code manually.',
          )
          return
        }
      ...
      try {
        ...
        if (!cancelled) {
          if (err.name === 'NotAllowedError') {
            setError(
              'Camera access was denied. Tap "Try Again" to re-prompt, or go to your browser settings to allow camera access for this site.',
            )
            ...
            setError(
              `Could not access camera (${err.name || 'unknown error'}). Please enter the code manually.`,
            )
          }
        }
      }
