...
export default function AdminHome({ crews = [], employees = [], onNav }) {
  ...
  useEffect(() => {
    getAttendanceToday()
      .then(data => {
        // API returns raw array — normalize to the shape this component expects
        if (Array.isArray(data)) {
          setAttendance({ crews: data, unrostered: [], totalWorking: 0, totalEmployees: 0 })
        } else {
          setAttendance(data)
        }
      })
  ...
  if (!employees || !crews) {
    return <div style={{ textAlign: 'center', padding: 40, color: C.textLight }}>Loading...</div>
  }
    ...
    <div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 10, marginBottom: 14 }}>
          <div tabIndex={0} role="button"
            ...
            style={{ ...cardStyle({ textAlign: 'center', padding: '16px 12px', cursor: 'pointer', marginBottom: 0 }),
              borderColor: statDetail === 'working' ? C.accent : C.cardBorder, transition: 'border-color 0.15s' }}>
          ...
          <div tabIndex={0} role="button"
            ...
            style={{ ...cardStyle({ textAlign: 'center', padding: '16px 12px', cursor: 'pointer', marginBottom: 0 }),
              borderColor: statDetail === 'unrostered' ? C.amber : C.cardBorder, transition: 'border-color 0.15s' }}>
          ...
          <div tabIndex={0} role="button"
            ...
            style={{ ...cardStyle({ textAlign: 'center', padding: '16px 12px', cursor: 'pointer', marginBottom: 0 }),
              borderColor: statDetail === 'rosters' ? C.blue : C.cardBorder, transition: 'border-color 0.15s' }}>
      ...
      {statDetail === 'working' && attendance && (
        <div style={{ ...cardStyle({ marginBottom: 14 }), borderColor: C.accentBorder, background: C.accentLight }}>
          <div style={{ fontSize: 14, fontWeight: 800, color: C.accent, marginBottom: 10 }}>Currently Working ({workingEmployees.length})</div>
          {workingEmployees.length === 0 ? (
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
              {attendance.crews.filter(c => c.submitted).map(crew => (
                <div key={crew.crewId}>
                  <div style={{ fontSize: 11, fontWeight: 700, color: C.accent, textTransform: 'uppercase', letterSpacing: 1, marginBottom: 4, marginTop: 4 }}>{crew.crewName}</div>
                  <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
                    {crew.members.map((name, i) => (
                      <span key={i} style={{ padding: '5px 10px', borderRadius: 8, background: '#fff', border: `1px solid ${C.accentBorder}`, fontSize: 13, fontWeight: 600, color: C.text }}>{name}</span>
                    ))}
                  </div>
                </div>
              ))}
      ...
      {statDetail === 'unrostered' && attendance && (
        <div style={{ ...cardStyle({ marginBottom: 14 }), borderColor: C.amberBorder, background: C.amberLight }}>
          <div style={{ fontSize: 14, fontWeight: 800, color: C.amber, marginBottom: 10 }}>Not on Any Roster Today ({attendance.unrostered.length})</div>
              ...
              {attendance.unrostered.map((emp, i) => (
                <div key={i} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '8px 12px', borderRadius: 10, background: '#fff', border: `1px solid ${C.amberBorder}` }}>
                  <div>
                    ...
                    <div style={{ fontSize: 12, color: C.textLight }}>{emp.defaultCrew ? `Usually on ${emp.defaultCrew}` : 'No default crew'}</div>
                  </div>
                </div>
              ))}
            </div>
      ...
      {statDetail === 'rosters' && attendance && (
        <div style={{ ...cardStyle({ marginBottom: 14 }), borderColor: C.blueBorder, background: C.blueLight }}>
          <div style={{ fontSize: 14, fontWeight: 800, color: C.blue, marginBottom: 10 }}>Roster Status</div>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
            {attendance.crews.map(crew => (
              <div key={crew.crewId} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '8px 12px', borderRadius: 10, background: '#fff', border: `1px solid ${crew.submitted ? C.accentBorder : C.amberBorder}` }}>
                <div>
                  ...
                  <div style={{ fontSize: 12, color: C.textLight }}>{crew.submitted ? `${crew.memberCount} member${crew.memberCount !== 1 ? 's' : ''} · by ${crew.submittedBy}` : 'Not submitted'}</div>
                </div>
        ...
        <div key={crew.crewId} style={{ marginBottom: 8 }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <div>
                ...
                <div style={{ fontSize: 13, color: C.textMed, marginTop: 2 }}>
                  {crew.submitted ? `${crew.memberCount} member${crew.memberCount !== 1 ? 's' : ''} · by ${crew.submittedBy}` : 'No roster submitted yet'}
                </div>
              </div>
              {crew.submitted
          ...
          {expanded === crew.crewId && crew.submitted && (
            <div style={{ background: '#FAFAF7', border: `1.5px solid ${C.accentBorder}`, borderTop: 'none', borderRadius: '0 0 16px 16px', padding: '12px 16px' }}>
              {crew.members.map((name, i) => (
                <div key={i} style={{ fontSize: 14, color: C.text, padding: '4px 0', fontWeight: 600 }}>{name}</div>
              ))}
            </div>
          )}
        </div>
