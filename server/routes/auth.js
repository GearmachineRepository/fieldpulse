...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    const existing = await db.query('SELECT id FROM admins WHERE LOWER(email) = LOWER($1)', [
      email.trim(),
    ])
    ...
    const orgResult = await db.query(
      `INSERT INTO organizations (name, slug, plan, trial_ends_at)
       ...
       RETURNING id`,
      [`${name.trim()}'s Organization`, slug, trialDays],
    )
    const orgId = orgResult.rows[0].id

    const r = await db.query(
      `INSERT INTO admins (name, email, role, pin_hash, supabase_uid, trial_ends_at, org_id)
       VALUES ($1, $2, 'owner', '', $3, NOW() + make_interval(days => $4), $5)
       RETURNING id, name, email, role`,
      [name.trim(), email.trim(), authData.user.id, trialDays, orgId],
    )
    ...
    if (signInError) {
      ...
      return res
        .status(500)
        .json({ error: 'Account created but sign-in failed. Please try logging in.' })
    }
...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (!admin.supabase_uid) {
      const adminWithHash = await db.query('SELECT password_hash FROM admins WHERE id = $1', [
        admin.id,
      ])
