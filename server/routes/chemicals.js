...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    const r = await db.query(
      'SELECT * FROM chemicals WHERE active = true AND org_id = $1 ORDER BY name',
      [orgId],
    )
    res.json(
      r.rows.map((row) => ({
  ...
  asyncHandler(async (req, res) => {
    ...
    const r = await db.query(
      ...
      [
        ...
        b.sdsUrl,
        b.labelUrl,
        wxJsonOrNull(b.wxTemp),
        wxJsonOrNull(b.wxHumidity),
        wxJsonOrNull(b.wxWind),
        wxJsonOrNull(b.wxConditions),
        orgId,
      ],
    )
...
router.delete(
  ...
  asyncHandler(async (req, res) => {
    const orgId = getOrgId(req)
    await db.query('UPDATE chemicals SET active = false WHERE id = $1 AND org_id = $2', [
      req.params.id,
      orgId,
    ])
    res.json({ success: true })
