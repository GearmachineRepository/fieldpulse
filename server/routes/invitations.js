...
const ALL_PAGES = [
  ...
  'training', 'certifications', 'incidents',
  'sds', 'documents', 'reports', 'projects', 'schedule',
]
...
router.post('/',
  ...
  asyncHandler(async (req, res) => {
    ...
    const r = await db.query(
      ...
      [orgId, email.trim().toLowerCase(), role, JSON.stringify(cleanPerms), adminId, token, expiresAt]
    )
...
router.post('/accept/:token',
  ...
  asyncHandler(async (req, res) => {
    ...
    const adminR = await db.query(
      ...
      [name.trim(), invite.email, invite.role, JSON.stringify(invite.permissions), invite.org_id, supabaseUid]
    )
    ...
    if (!authToken) {
      authToken = signToken({ type: 'admin', adminId: newAdmin.id, role: newAdmin.role, orgId: invite.org_id })
    }
    ...
    logger.info({ adminId: newAdmin.id, email: invite.email, role: invite.role }, 'Invitation accepted')
