...
router.delete(
  ...
  asyncHandler(async (req, res) => {
    const orgId = getOrgId(req)
    await db.query('UPDATE account_groups SET active = false WHERE id = $1 AND org_id = $2', [
      req.params.id,
      orgId,
    ])
    await db.query('UPDATE accounts SET group_id = NULL WHERE group_id = $1 AND org_id = $2', [
      req.params.id,
      orgId,
    ])
    res.json({ success: true })
