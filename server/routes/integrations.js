...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (conn.rows.length === 0 || !conn.rows[0].config?.api_key) {
      return res
        .status(400)
        .json({ error: 'SDS Manager not connected. Add your API key in Settings.' })
    }
    ...
    res.json({
      results: [],
      message:
        'SDS Manager search connected — real API results will appear when the integration goes live.',
    })
  }),
)
...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    res.json({
      success: true,
      synced: 0,
      message: 'Sync infrastructure ready — records will sync when the integration goes live.',
    })
  }),
)
