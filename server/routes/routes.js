...
export default function routeRoutes(upload, uploadToStorage) {
  ...
  router.get(
    ...
    asyncHandler(async (req, res) => {
      ...
      if (start) {
        params.push(start)
        where.push(`rc.work_date >= $${params.length}`)
      }
      if (end) {
        params.push(end)
        where.push(`rc.work_date <= $${params.length}`)
      ...
      if (routeId) {
        params.push(routeId)
        where.push(`rc.route_id = $${params.length}`)
      }
  ...
  router.get(
    ...
    `,
      ...
      for (const row of result.rows) {
        ...
        schedule[key].routes.push({
          id: row.id,
          name: row.name,
          color: row.color,
          crewId: row.crew_id,
          crewName: row.crew_name,
          stopCount: parseInt(row.stop_count),
        })
      }
    ...
    `,
      ...
      res.json(
        result.rows.map((row) => ({
          ...
          completedStops: parseInt(row.completed_stops),
        })),
      )
    }),
  )
    ...
    `,
      ...
      while (current <= endDate) {
        ...
        for (const stop of dayStops) {
          ...
          // Date range check
          const stopStart = stop.stop_start_date
            ? new Date(stop.stop_start_date + 'T12:00:00')
            : new Date('2000-01-01')
          const stopEnd = stop.stop_end_date
            ? new Date(stop.stop_end_date + 'T12:00:00')
            : new Date('2099-12-31')
          ...
          if (interval > 1) {
            const epoch = stop.stop_start_date
              ? new Date(stop.stop_start_date + 'T12:00:00')
  ...
  router.post(
    ...
    requireAuth,
    validateBody({
      routeStopId: { required: true, type: 'number' },
      routeId: { required: true, type: 'number' },
      completedByName: { required: true, type: 'string' },
    }),
    asyncHandler(async (req, res) => {
      const {
        routeStopId,
  ...
  router.post(
    '/completions/:completionId/photos',
    requireAuth,
    upload.array('photos', 10),
    uploadToStorage,
    asyncHandler(async (req, res) => {
      const completionId = parseInt(req.params.completionId, 10)
      if (isNaN(completionId) || completionId < 1)
  ...
  router.get(
    ...
    asyncHandler(async (req, res) => {
      ...
      if (dayOfWeek !== undefined) {
        ...
        where.push(`r.day_of_week = $${params.length}`)
      }
      const result = await db.query(
        `
  ...
  router.post(
    ...
    asyncHandler(async (req, res) => {
      ...
      const result = await db.query(
        'INSERT INTO routes (name, crew_id, day_of_week, color, notes, org_id) VALUES ($1,$2,$3,$4,$5,$6) RETURNING *',
        [
          name,
          crewId || null,
  ...
  router.get(
    ...
    asyncHandler(async (req, res) => {
      ...
      const routeR = await db.query(
        'SELECT r.*, c.name AS crew_name FROM routes r LEFT JOIN crews c ON c.id = r.crew_id WHERE r.id = $1 AND r.active = true AND r.org_id = $2',
        [req.params.id, orgId],
      )
  ...
  router.put(
    ...
    asyncHandler(async (req, res) => {
      ...
      const { name, crewId, dayOfWeek, color, notes } = req.body
      await db.query(
        'UPDATE routes SET name=$1, crew_id=$2, day_of_week=$3, color=$4, notes=$5 WHERE id=$6 AND org_id=$7',
        [
          name,
          crewId || null,
          dayOfWeek !== undefined ? dayOfWeek : null,
  ...
  router.delete(
    ...
    asyncHandler(async (req, res) => {
      const orgId = getOrgId(req)
      await db.query('UPDATE routes SET active = false WHERE id = $1 AND org_id = $2', [
        req.params.id,
        orgId,
      ])
      res.json({ success: true })
  ...
  router.get(
    ...
    `,
      ...
      route.progress = {
        completed: route.stops.filter((s) => s.completion).length,
  ...
  router.post(
    ...
    requireAuth,
    validateIdParam,
    validateBody({ accountId: { required: true, type: 'number' } }),
    asyncHandler(async (req, res) => {
      const {
        accountId,
        estimatedMinutes,
      ...
      const maxR = await db.query(
        'SELECT COALESCE(MAX(stop_order), -1) + 1 AS next_order FROM route_stops WHERE route_id = $1 AND active = true',
        [req.params.id],
      )
      const result = await db.query(
  ...
  router.delete(
    ...
    asyncHandler(async (req, res) => {
      const stopId = parseInt(req.params.stopId, 10)
      if (isNaN(stopId) || stopId < 1) return res.status(400).json({ error: 'Invalid stop ID' })
      await db.query('UPDATE route_stops SET active = false WHERE id = $1 AND route_id = $2', [
        stopId,
        req.params.id,
      ])
      res.json({ success: true })
  ...
  router.post(
    ...
    asyncHandler(async (req, res) => {
      ...
      if (!exceptionType || !dateStart)
        return res.status(400).json({ error: 'exceptionType and dateStart required' })
      const result = await db.query(
      ...
      res.json({
        id: result.rows[0].id,
        type: result.rows[0].exception_type,
...
function isStopDueOnDate(stop, dateObj, dateStr) {
  ...
  if (interval > 1) {
    const epoch = stop.startDate
      ? new Date(stop.startDate + 'T12:00:00')
      : new Date('2024-01-01T12:00:00')
    const msPerWeek = 7 * 24 * 60 * 60 * 1000
...
function formatRoute(row) {
  return {
    ...
    color: row.color,
    notes: row.notes,
    stopCount: row.stop_count ? parseInt(row.stop_count) : 0,
    totalMinutes: row.total_minutes ? parseInt(row.total_minutes) : 0,
...
function formatStop(row) {
  return {
    ...
    account: {
      ...
      estimatedMinutes: row.account_estimated_minutes || 30,
    },
  }
}
