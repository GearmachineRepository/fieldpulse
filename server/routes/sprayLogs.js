...
export default function sprayLogRoutes(upload, uploadToStorage) {
  ...
  router.get(
    ...
    asyncHandler(async (req, res) => {
      ...
      params.push(orgId)
      where.push(`sl.org_id = $${params.length}`)

      if (vehicleId) {
        params.push(vehicleId)
        where.push(`sl.vehicle_id = $${params.length}`)
      }
      if (crewName) {
        params.push(crewName)
        where.push(`sl.crew_name = $${params.length}`)
      }
      if (start) {
        params.push(start)
        where.push(`sl.created_at >= $${params.length}::date`)
      }
      if (end) {
        params.push(end)
        where.push(`sl.created_at < ($${params.length}::date + interval '1 day')`)
      }
