...
export default function fieldDocRoutes(upload, uploadToStorage) {
  ...
  router.get(
    ...
    asyncHandler(async (req, res) => {
      ...
      params.push(orgId)
      where.push(`fd.org_id = $${params.length}`)
      if (type) {
        params.push(type)
        where.push(`fd.type = $${params.length}`)
      }
      if (crewName) {
        params.push(crewName)
        where.push(`fd.crew_name = $${params.length}`)
      }
      if (status) {
        params.push(status)
      ...
      if (req.query.assetType) {
        ...
        where.push(`fd.asset_type = $${params.length}`)
      }
      if (req.query.assetId) {
        params.push(req.query.assetId)
        where.push(`fd.asset_id = $${params.length}`)
      }
  ...
  router.post(
    ...
    asyncHandler(async (req, res) => {
      ...
      const check = await db.query('SELECT id FROM field_docs WHERE id = $1 AND org_id = $2', [
        req.params.id,
        orgId,
      ])
