...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (crewId) {
      params.push(crewId)
      where.push(`e.crew_id = $${params.length}`)
    }
...
router.post(
  ...
  asyncHandler(async (req, res) => {
    const orgId = getOrgId(req)
    const { title, notes, eventDate, startTime, endTime, eventType, color, crewId, accountId } =
      req.body
    const result = await db.query(
...
router.put(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (crewId !== undefined) {
      params.push(crewId)
      updates.push(`crew_id = $${params.length}`)
    }
    ...
    params.push(orgId)
    await db.query(
      `UPDATE schedule_events SET ${updates.join(', ')} WHERE id = $${params.length - 1} AND org_id = $${params.length}`,
      params,
    )
...
router.delete(
  ...
  asyncHandler(async (req, res) => {
    const orgId = getOrgId(req)
    await db.query('UPDATE schedule_events SET active = false WHERE id = $1 AND org_id = $2', [
      req.params.id,
      orgId,
    ])
    res.json({ success: true })
