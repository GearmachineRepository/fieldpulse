...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    const maxOrder = await db.query(
      'SELECT COALESCE(MAX(sort_order), 0) + 1 AS next FROM resource_categories WHERE org_id = $1',
      [orgId],
    )
    const r = await db.query(
...
router.put(
  ...
  asyncHandler(async (req, res) => {
    ...
    const { name, icon, color } = req.body
    await db.query(
      'UPDATE resource_categories SET name=$1, icon=$2, color=$3 WHERE id=$4 AND org_id=$5',
...
router.delete(
  ...
  asyncHandler(async (req, res) => {
    const orgId = getOrgId(req)
    await db.query('UPDATE resource_categories SET active = false WHERE id = $1 AND org_id = $2', [
      req.params.id,
      orgId,
    ])
    await db.query(
      'UPDATE resources SET category_id = NULL WHERE category_id = $1 AND org_id = $2',
      [req.params.id, orgId],
    )
    res.json({ success: true })
  }),
...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (pinned === 'true') {
      where.push('r.pinned = true')
    }
    if (search) {
      params.push(`%${search}%`)
      where.push(
        `(r.title ILIKE $${params.length} OR r.description ILIKE $${params.length} OR r.original_name ILIKE $${params.length})`,
      )
    }
...
router.delete(
  ...
  asyncHandler(async (req, res) => {
    const orgId = getOrgId(req)
    await db.query('UPDATE resources SET active = false WHERE id = $1 AND org_id = $2', [
      req.params.id,
      orgId,
    ])
    res.json({ success: true })
...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    const res2 = await db.query('SELECT id FROM resources WHERE id = $1 AND org_id = $2', [
      req.params.id,
      orgId,
    ])
