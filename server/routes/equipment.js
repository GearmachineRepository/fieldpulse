...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    const r = await db.query(
      'SELECT * FROM equipment WHERE active = true AND org_id = $1 ORDER BY name',
      [orgId],
    )
    res.json(r.rows)
  }),
...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    const r = await db.query(
      'INSERT INTO equipment (name, type, org_id) VALUES ($1, $2, $3) RETURNING *',
      [name, type, orgId],
    )
    res.json(r.rows[0])
  }),
...
router.put(
  ...
  asyncHandler(async (req, res) => {
    const orgId = getOrgId(req)
    const { name, type } = req.body
    await db.query('UPDATE equipment SET name = $1, type = $2 WHERE id = $3 AND org_id = $4', [
      name,
      type,
      req.params.id,
      orgId,
...
router.delete(
  ...
  asyncHandler(async (req, res) => {
    const orgId = getOrgId(req)
    await db.query('UPDATE equipment SET active = false WHERE id = $1 AND org_id = $2', [
      req.params.id,
      orgId,
    ])
    res.json({ success: true })
