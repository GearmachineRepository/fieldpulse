...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (accountId) {
      ...
      where.push(`sp.account_id = $${params.length}`)
    }
    if (routeId) {
      params.push(routeId)
      where.push(`sp.route_id = $${params.length}`)
    }
  ...
  asyncHandler(async (req, res) => {
    ...
    const result = await db.query(
      ...
      [
        accountId,
        frequency,
        intervalWeeks || getDefaultInterval(frequency),
        preferredDays || [1],
        routeId || null,
...
router.put(
  ...
  asyncHandler(async (req, res) => {
    ...
    const old = await db.query(
      'SELECT route_id, account_id FROM service_plans WHERE id = $1 AND org_id = $2',
      [req.params.id, orgId],
    )
...
router.patch(
  ...
  asyncHandler(async (req, res) => {
    ...
    await db.query(
      'UPDATE service_plans SET status = $1, updated_at = NOW() WHERE id = $2 AND org_id = $3',
      [status, req.params.id, orgId],
    )

    if (status === 'canceled') {
      await db.query(
        'UPDATE service_plans SET end_date = COALESCE(end_date, CURRENT_DATE) WHERE id = $1 AND org_id = $2',
        [req.params.id, orgId],
      )
    }
...
router.delete(
  ...
  asyncHandler(async (req, res) => {
    const orgId = getOrgId(req)
    await db.query(
      'UPDATE service_plans SET active = false, updated_at = NOW() WHERE id = $1 AND org_id = $2',
      [req.params.id, orgId],
    )
    res.json({ success: true })
  }),
...
router.post(
  ...
  validateIdParam,
  validateBody({
    exceptionType: { required: true, type: 'string' },
    dateStart: { required: true, type: 'string' },
  }),
  asyncHandler(async (req, res) => {
    const orgId = getOrgId(req)

    // Verify plan belongs to this org
    const check = await db.query('SELECT id FROM service_plans WHERE id = $1 AND org_id = $2', [
      req.params.id,
      orgId,
    ])
  ...
  asyncHandler(async (req, res) => {
    ...
    for (const plan of plans) {
      ...
      const planStart = plan.start_date
        ? new Date(plan.start_date + 'T12:00:00')
        : new Date('2000-01-01')
    ...
    visits.sort(
      (a, b) => a.date.localeCompare(b.date) || a.accountName.localeCompare(b.accountName),
    )
    res.json(visits)
  }),
)
...
async function autoAddStop(routeId, accountId) {
  try {
    const existing = await db.query(
      'SELECT id FROM route_stops WHERE route_id = $1 AND account_id = $2 AND active = true',
      [routeId, accountId],
    )
