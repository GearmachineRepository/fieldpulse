...
router.get(
  ...
  asyncHandler(async (req, res) => {
    const crews = await db.query(
      'SELECT id, name, lead_name FROM crews WHERE active = true ORDER BY name',
    )
...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    const r = await db.query(
      'SELECT id, name, lead_name FROM crews WHERE active = true AND org_id = $1 ORDER BY name',
      [orgId],
    )
    res.json(r.rows)
  }),
...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    const r = await db.query(
      'INSERT INTO crews (name, lead_name, org_id) VALUES ($1, $2, $3) RETURNING *',
      [name, leadName, orgId],
    )
    res.json(r.rows[0])
  }),
...
router.delete(
  ...
  asyncHandler(async (req, res) => {
    const orgId = getOrgId(req)
    await db.query('UPDATE crews SET active = false WHERE id = $1 AND org_id = $2', [
      req.params.id,
      orgId,
    ])
    res.json({ success: true })
