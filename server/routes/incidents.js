...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    const check = await db.query('SELECT id FROM incidents WHERE id = $1 AND org_id = $2', [
      req.params.id,
      orgId,
    ])
    if (check.rows.length === 0) throw new AppError('Incident not found', 404)
...
router.delete(
  ...
  asyncHandler(async (req, res) => {
    ...
    await db.query('DELETE FROM incident_photos WHERE id = $1 AND org_id = $2', [photoId, orgId])
